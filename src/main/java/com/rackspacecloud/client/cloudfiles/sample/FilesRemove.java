...
public class FilesRemove {
    ...
    private static void removeObject(String objectNameWithPath)
            throws HttpException, IOException, FilesException {
        if (!StringUtils.isNotBlank(objectNameWithPath)) {
            System.err.println(
                    "You must provide a valid value for the  Object name and path !");
            System.exit(0);
        } // if (!StringUtils.isNotBlank(ObjectNameWithPath))
        int firstSlashLocation = objectNameWithPath.indexOf('/');
        if (firstSlashLocation > -1) {
            String container = objectNameWithPath.substring(0,
                    firstSlashLocation - 1);
            String object = objectNameWithPath
                    .substring(firstSlashLocation + 1);
            ...
            System.err.println(
                    "Please specify the object path in the form containerName/objectName");
            System.exit(-1);
        }
    }
    ...
    private static void removeContainer(String containerName, boolean recurse)
        ...
        if (!StringUtils.isNotBlank(containerName)) {
            System.out.println(
                    "You must provide a valid value for the  Container name !");
            logger.fatal(
                    "You must provide a valid value for the  Container name !");
            System.exit(0);
        } // if (!StringUtils.isNotBlank(containerName))
    ...
    private static Options addCommandLineOptions() {
        ...
        Option recurse = new Option("r",
                "Recursively go through the folders and files");
