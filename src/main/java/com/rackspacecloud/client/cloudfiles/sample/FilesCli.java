...
public class FilesCli {
        ...
        if ("get".equals(command)) {
            if (components.length == 1) {
                try {
                    ...
                    System.out.println("The account has " + nContainers
                            + ((nContainers == 1) ? " container"
                                    : " containers"));
                ...
                if (slashLocation == -1) {
                    ...
                    try {
                        ...
                        System.out.println(
                                "Error trying to list container contents");
                        e.printStackTrace();
                        return true;
                    }
                }
                    ...
                    if (components.length != 3) {
                        System.out.println(
                                "usage:  get container/filename.ext destination.ext");
                        return true;
                    }
                    ...
                    InputStream is=null;
                    try {
                        is = client.getObjectAsStream(container,
                                object);
                        ...
                        System.out.println(
                                name + " downlaoded to " + destination);
                    }
                    catch (Exception ex) {
        ...
        if ("head".equals(command)) {
            if (components.length == 1) {
                try {
                    ...
                    System.out.println(
                            "    Total Account Size: " + info.getBytesUsed());
                    System.out.println();
                }
                catch (Exception e) {
            ...
            for (int i = 1; i < components.length; i++) {
                ...
                if (slashLocation == -1) {
                    ...
                    try {
                        FilesContainerInfo containerInfo = client
                                .getContainerInfo(name);
                        System.out.println("Information for " + name);
                        System.out.println("  Object Count: "
                                + containerInfo.getObjectCount());
                        System.out.println("    Total Size: "
                                + format.format(containerInfo.getTotalSize())
                                + " bytes");
                        System.out.println();
                    }
                    catch (Exception e) {
                    ...
                    String object = name.substring(slashLocation + 1);
                    try {
                        FilesObjectMetaData metadata = client
                                .getObjectMetaData(container, object);
                        /*if (metadata == null) {
                            System.out.println(
                                    "Could not get metadata for " + name);
                        }
                        else {*/
                            System.out.println("LGV: " + container + ":"
                                    + object + ":" + metadata);
                            System.out.println("Information for " + name);
                            System.out.println("  Total Size: "
                                    + metadata.getContentLength() + " bytes");
                            System.out.println(
                                    "  MIME type: " + metadata.getMimeType());
        ...
        if ("put".equals(command)) {
            if (components.length == 2) {
                ...
                if (newContainerName.indexOf('/') != -1) {
                    System.out
                            .println("Container names may not contain slashes");
                    return true;
                }
                try {
                ...
                try {
                    ...
                    if (client.storeObject(containerName, file,
                            mimeType) != null) {
                        System.out.println(
                                "Object " + file.getName() + " was created");
                    }
                    return true;
                }
                catch (Exception e) {
                ...
                if (slashLocation == -1) {
                    ...
                    try {
                        ...
                        System.out.println(name
                                + " was not empty.  Please delete the contents and try again");
                    }
                    catch (Exception e) {
                    ...
                    if (returnCode) {
                        ...
                        System.out.println(
                                "Unexpected result deleting container ");
                    }
                }
                else {
                    // object
    ...
    public static class CommandLineOptions {
        ...
        public CommandLineOptions(String[] args) {
                ...
                if ("username".equals(args[i])) {
                    if (i >= args.length - 1) {
                        throw new RuntimeException(
                                "No argument following option 'username'.");
                    }
                    userName = args[i + 1];
                    i++;
                }
                    ...
                    if (i >= args.length - 1) {
                        throw new RuntimeException(
                                "No argument following option 'password'.");
                    }
                    password = args[i + 1];
                    i++;
                }
            ...
            if (userName == null) {
                throw new RuntimeException(
                        "No username specified (use option 'username' or set CLOUDFILES_USERNAME environment variable).");
            }
            if (password == null) {
                throw new RuntimeException(
                        "No password specified (use option 'password' or set CLOUDFILES_PASSWORD environment variable).");
            }
    ...
    public static void parseArgs(String[] args) {
        try {
            ...
            if (options.command.length == 0) {
                System.out.println(
                        "Login was successful, but no other commands were specified.");
                System.out.println(HELP_STRING);
            }
            else {
    ...
    public static void interactiveMode() {
        ...
        if (commandLine.doLogin()) {
            ...
            String cmd = "";
            do {
                FilesClient tmpClient=commandLine.client;
                if(tmpClient !=null) {
                    String account = tmpClient.getAccount();
                    account = (account == null) ? tmpClient.getUserName()
                            : account;
                    System.out.print(account + ": ");
                    try {
                        BufferedReader reader = commandLine.console;
                        if(reader!=null){
                            cmd = reader.readLine();
                        }
                    }
            ...
            while (commandLine!=null && commandLine.evaluateCommand(cmd));
            System.exit(0);
        }
        else {
