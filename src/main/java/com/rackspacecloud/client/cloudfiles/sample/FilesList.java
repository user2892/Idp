...
/*
 * TODO: Add List Metadata for all objects as a switch on the all TODO: Add list
 * Metadata for one object as a switch on a specific object of a container TODO:
 * Add list specific Objects of a container
 */
public class FilesList {
    ...
    private static void printContainer(String containerName,
        ...
        if (client.login()) {
                ...
                if (value.getName().equalsIgnoreCase(containerName)) {
                    ...
                    if (humanReadable) {
                        System.out.println("\tTotal Size: "
                                + info.getTotalSize() / 1024 + "KB\n");
                    }
                    else
                        System.out.println("\tTotal Size: "
                                + info.getTotalSize() + "Bytes\n");
                } // if (value.getName().equalsIgnoreCase(containerName))
                else
                    notFound = true;
            } // end for
            if (notFound)
                System.out.println(
                        "Container: " + containerName + " was not found !");
        }
    }// end private static void printContainersAndContent () throws IOException,
     // HttpException, FilesAuthorizationException

    private static void printContainersAll(boolean humanReadable)
            throws IOException, HttpException, FilesException {
        FilesClient client = new FilesClient();
        if (client.login()) {
            List<FilesContainer> containers = client.listContainers();
            ...
            for (FilesContainer value : containers) {
                ...
                if (humanReadable) {
                    ...
                    System.out.println(
                            "\tTotal Size: " + info.getTotalSize() + "Bytes\n");
            }
        }
    }// end private static void printContainersAndContent () throws IOException,
     // HttpException, FilesAuthorizationException

    private static void printContainers(boolean humanReadable)
            throws IOException, HttpException, FilesException {
        FilesClient client = new FilesClient();
        if (client.login()) {
            List<FilesContainer> containers = client.listContainers();
            ...
            for (FilesContainer value : containers) {
                ...
                if (humanReadable) {
                    System.out.println("\tTotal Size: "
                            + getSizeString(info.getTotalSize()) + "\n");
                }
                else
                    System.out.println(
                            "\tTotal Size: " + info.getTotalSize() + "Bytes\n");
    ...
    private static Options addCommandLineOptions() {
        ...
        Option containersOnly = OptionBuilder.hasArg(false)
                .withDescription(
                        "Show ONLY Containers in account.  No Object information is provided.")
                .create("containersOnly");
        Option containers = OptionBuilder.hasArg(false)
                .withDescription(
                        "Show Containers and their objects in this account")
                .create("all");
        Option container = OptionBuilder.withArgName("container").hasArg(true)
                .withDescription(
                        "Show Containers and their objects in this account")
                .create("container");
        Option kb = new Option("H", "humanReadable", false,
                "Show size of objects in human readable form.  On its own this has no meaning and needs to be used in conjuction to a command that provides size of objects and containers");
